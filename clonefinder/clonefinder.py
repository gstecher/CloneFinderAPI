       ...
       if status != True:
        ...
        while Repeat=='y':	
            ...
            if Message[:10]!='decomposed' or DecomTuNum>(1.0*len(v_obs)/2):  
                       ...
                       hit_seq_build = Align.UpMeg(hit_seq_dic,hit_seq_dic.keys())					   
                       id = 'dec_mega_alignment' 				   
                       status = tree_builder.do_mega_mp(hit_seq_build, id)
                       if status == True:
                             decomseqs_with_ancestor, A1 = tree_builder.alignment_least_back_parallel_muts(True) # True will execute code to remove redundant seqs (True is default)
        ...
        if status == True:
                A1, tree = tree_builder.alignment_least_back_parallel_muts(True) 
                Rooted=AnalyzeTree.RootTree(tree)
                OutFile.GetOut(params.input_id + '_CloneFinder.nwk', Rooted.replace('hg19:','Normal:'))				                     
