...
if parser.parse() == False:
        ...
        while RepeatUnion=='y':	
          ...
          #  print seqs_with_ancestor
           ...
           # clusters.estimatedSNV_save_to_file()			
            ...
            decomposition = SNPGroupCombiner(tumor2cluster, seqs_with_ancestor, tumor_seqs, tsp_list, params.freq_cutoff, params.option_b.split(',')[1].strip(), params.option_b.split(',')[2].strip())
        ...
        # 'rename clone anmes'
        seqs_with_ancestor_newName, CloFre_seqs_with_ancestor_newName, clone_order  = OutFile.ReNameCloFreMeg(seqs_with_ancestor, CloFre_seqs_with_ancestor, 'number')		
       # Align.save_mega_alignment_to_file('Test_dec_rename.meg', seqs_with_ancestor_newName)
        ...
        if params.option_a=='Off' and params.option_c=='Off' and params.option_d=='Off': pass
            ...
            if params.option_d=='Off': pass
                ...
                seqs_with_ancestor_newName, CloFre_seqs_with_ancestor_newName, clone_order  = OutFile.ReNameCloFreMeg(add_inter_ancester_seq, add_inter_ancester_clone_frequency, 'number')	          				
              #  Align.save_mega_alignment_to_file('Test_Anc.meg', seqs_with_ancestor_newName)				

            ##with binomial test			  
#            print 'binomial significance test'
 #           Align.save_mega_alignment_to_file('Test_bino_before.meg', seqs_with_ancestor_newName)
  #          CloFreAna.save_frequency_table_to_file('test_bino_before.txt', CloFre_seqs_with_ancestor_newName,  [])				
   #         significant_clones = SignificanceTester(params.bino_num, params.bino_cutoff, params.error_rate, tsp_list, seqs_with_ancestor_newName, CloFre_seqs_with_ancestor_newName)
    #        significant_clone_in_tumor, significant_clone_list = significant_clones.count_detection()
     #       clone_list, clone_seq_dic=Align.name2seq(seqs_with_ancestor_newName)	
      #      significant_clone_seq0 =Align.UpMeg(clone_seq_dic, clone_list)	
       #     Align.save_mega_alignment_to_file('Test_bino.meg', significant_clone_seq0)
        #    CloFreAna.save_frequency_table_to_file('test_bino.txt', significant_clone_in_tumor,  [])

#            print 'estimate clone frequencies of significant clones'	
 #           CloFreCom = CloneFrequencyComputer(significant_clone_seq0, tsp_list, params.freq_cutoff)
  #          significant_seq, significant_clone_frequency = CloFreCom.regress_hit_clone(significant_clone_in_tumor)
   #         significant_seq+=['#hg19','A'*len(Initial_Good_posi_info)]	
    #        Align.save_mega_alignment_to_file('Test_bino1.meg', significant_seq)
     #       CloFreAna.save_frequency_table_to_file('test_bino1.txt', significant_clone_frequency,  [])

            ##no binomial test			
            significant_clone_frequency = {}
            for Tu in CloFre_seqs_with_ancestor_newName:
               significant_clone_frequency['T-'+Tu]= CloFre_seqs_with_ancestor_newName[Tu]			
            ...
            Align.save_mega_alignment_to_file('Test_final.meg', significant_seq)
            CloFreAna.save_frequency_table_to_file('test_final.txt', significant_clone_frequency,  [])	
            ...
            print 'binomial significance test'			
            significant_clones = SignificanceTester(params.bino_num, params.bino_cutoff, params.error_rate, tsp_list, final_seq1, final_clone_frequency1)
