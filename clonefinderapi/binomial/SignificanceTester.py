...
class SignificanceTester():
    ...
    """
    def __init__(self, rep_number, significance_cutoff, error_rate, tsp_list, sequences, original_clone_frequency):
        self.RepNum = rep_number
        self.Sig = significance_cutoff		
        self.CutOff = error_rate		
    ...
    def count_detection(self):
        ...
        while RepID<=self.RepNum: 
            ...
            clone_frequency = CloneFrequencyComputer(self.sequences, tsp_rep_list)
            clone_frequency_table = clone_frequency.regress()			
            for tumor in clone_frequency_table:
                ...
                for clone in clone_frequencies:
                    if RepID==1: self.hit_count[tumor][clone] = 0
                    if clone_frequencies[clone]>self.CutOff: self.hit_count[tumor][clone]+=1.0/self.RepNum								
            RepID += 1			
     #   print self.hit_count
        ...
        for tumor in self.hit_count:
            ...
            for clone in hit_count:
                ...
                else: self.hit_count1[tumor][clone]=0	
            if CloneHit==0:
                LargeCloneFre = find_large_clone_frequency(tumor)
    ...
    def find_large_clone_frequency(tumor):
        OriClo2Fre=self.OriCloFre[tumor]	
        Lar=0
        LarClo=[]
